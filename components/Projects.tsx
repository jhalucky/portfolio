...
export default function Projects({ showAll = false }: { showAll?: boolean }) {
  ...
  const visibleProjects = showAll || expanded ? projects : projects.slice(0, 4);
   ...
   useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      return () => ctx.revert(); // ✅ clean GSAP context
    }
  }, []);
  ...
  <div className="flex flex-col gap-5">
    ...
    {/* ✅ Added wrapper class for GSAP trigger */}
    <div className="projects-section grid gap-6 sm:grid-cols-2">
      {visibleProjects.map((project) => (
        <div key={project.id} className="animated-border">
          <article
          ...
          >
            <div>
              <img
                rel="icon"
                src={project.favicon}
                className="mb-2 bg-gray-300 rounded-full h-20"
              />
            </div>
